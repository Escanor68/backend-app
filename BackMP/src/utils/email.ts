...
const simulateEmailSending = async (options: EmailOptions): Promise<void> => {
    ...
    console.log(
        '   Texto:',
        options.text?.substring(0, 100) + '...' || 'No text',
    );
    ...
    if (options.attachments && options.attachments.length > 0) {
        ...
        options.attachments.forEach((attachment, index) => {
            console.log(
                `   ${index + 1}. ${attachment.filename} (${attachment.path})`,
            );
        });
    }
};
